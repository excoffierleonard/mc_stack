...
new_dir="minecraft_server_$new_id"
new_stack_compose_file="$new_dir/compose.yaml"
...
# Calculate new ports
increment=3
new_server_port=$((base_server_port + new_id * increment))
new_rcon_port=$((base_rcon_port + new_id * increment))
new_sftp_port=$((base_sftp_port + new_id * increment))

# Update the .env file in the new directory
sed -i "s/^MINECRAFT_SERVER_SERVICE=.*/MINECRAFT_SERVER_SERVICE=minecraft_server_$new_id/" "$new_dir/.env"
sed -i "s/^MINECRAFT_SERVER_VOLUME=.*/MINECRAFT_SERVER_VOLUME=minecraft_server_$new_id/" "$new_dir/.env"
sed -i "s/^MINECRAFT_SERVER_NETWORK=.*/MINECRAFT_SERVER_NETWORK=minecraft_server_$new_id/" "$new_dir/.env"
...
sed -i "s/^SFTP_SERVER_SERVICE=.*/SFTP_SERVER_SERVICE=sftp_server_$new_id/" "$new_dir/.env"
...
echo "New MINECRAFT_SERVER_SERVICE: minecraft_server_$new_id"
echo "New MINECRAFT_SERVER_VOLUME: minecraft_server_$new_id"
echo "New MINECRAFT_SERVER_NETWORK: minecraft_server_$new_id"
echo "New SFTP_SERVER_SERVICE: sftp_server_$new_id"
